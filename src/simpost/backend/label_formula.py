...
def format_curve_label(
    ...
    Supported syntax:
    - variables: `curve_label`, `parameter`, `file_name`, or their quoted forms
    ...
    if _uses_curve_label(expression) and not curve_label.strip():
        return fallback
    if _uses_file_name(expression) and not file_name.strip():
        return fallback

    try:
        parts = [
            _evaluate_term(term, curve_label, parameter, file_name)
...
def _uses_file_name(expression: str) -> bool:
    return "file_name" in expression
...
def _evaluate_term(term: str, curve_label: str, parameter: str, file_name: str) -> str:
    base, removals = _split_removal(term)
    value = _term_value(base, curve_label, parameter, file_name)
...
def _term_value(term: str, curve_label: str, parameter: str, file_name: str) -> str:
